    ...
    methods: {
        ...
        async createChatSession() {
            ...
            try {
                ...
                const type = this.mapDeviceType(device?.from);
                ...
                setTimeout(() => {
                    this.$router.push({
                        ...
                        params: {
                            ...
                            chatid: response.data.contact.id,
                        },
                        query: {
                            ...(isWaba ? { tpl: '1' } : {}),
                        },
        ...
        */
        ...
        async saveContact() {
            ...
            if (!device) {
                const m = 'Device tidak valid';
                this.$showToast ? this.$showToast(m, 'error', 3000) : alert(m);
                return;
            }

            // Normalisasi nomor (jaga-jaga paste tanpa lewat @input)
            let phone = String(this.newContact.phone).replace(/[^0-9]/g, '');
            ...
            try {
                ...
                this.$showToast && this.$showToast('Kontak berhasil ditambahkan', 'success', 3000);

                this.resetAddContact();
                const isWaba = type === 'waba';
                this.startChat(data.contact, isWaba);
            } catch (error) {
        ...
        validatePhoneInput(event) {
            ...
            if (v.startsWith('620')) v = '62' + v.slice(3);
            this.newContact.phone = v;
        },

        /**
         * Mapping device type ke backend type (6 channel)
         */
        mapDeviceType(from) {
            const map = {
                unofficial: 'whatsapp',
