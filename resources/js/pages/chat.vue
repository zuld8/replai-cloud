            ...
            <div class="chat-header">
                <div class="user-details">
                    ...
                    <div class="avatar avatar-lg online flex-shrink-0">
                       <img
                            :src="detail.photo || '/images/user.png'"
                            ...
                            <ul class="dropdown-menu dropdown-menu-end p-3">
                                <li>
                                    <router-link
                                        ...
                                        class="dropdown-item"
                                        ><i class="ti ti-x me-2"></i>Close Message</router-link
                                    >
                                </li>
                            </ul>
                        </li>
...
export default {
    ...
    methods: {
        ...
        async getMessages(lastChat = "") {
            try {
                const response = await this.$axios.get(
                    `/device/chats/detail/${this.$route.params.id}/${this.$route.params.chatid}?last_id=${lastChat}`
                );
                ...
                if (data.autoread) {
                    var toMarkRead = listData
                        .filter(
                            (newItem) =>
                                newItem.status === "UNREAD" && !newItem.sender
                        )
                        .map((filteredItem) => ({
                    ...
                    if (toMarkRead.length > 0) {
                        this.markReadMessages(toMarkRead);
                    }
                }
            } catch (error) {
        ...
        async markReadMessages(messages) {
            try {
                const response = await this.$axios.post(
                    `/device/misc/read-messages/${this.$route.params.id}`,
                    {
                        chatid: this.chatId,
                        messages: messages,
                    }
