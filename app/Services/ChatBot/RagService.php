...
class RagService
{
    ...
    public function processDocument(FineTunnel $fineTunnel, UploadedFile $file): array
    {
        ...
        $filePath = $file->storeAs("uploads/folders/{$fineTunnel->business->id}/ai-training/{$fineTunnel->id}", $filename);
    ...
    protected function extractPdfEmbeddedImages($page, int $pageNum, FineTunnelDocument $document): array
    {
        ...
        try {
            ...
            $imageDir = public_path("uploads/folders/{$document->fineTunnel->business->id}/ai-training/{$document->fineTunnel->id}");
            ...
            foreach ($xObjects as $xObjectName => $xObject) {
                try {
                    ...
                    $imagePath = "uploads/folders/{$document->fineTunnel->business->id}/ai-training/{$document->fineTunnel->id}/" . $imageName;
    ...
    public function searchSimilarChunks(FineTunnel $fineTunnel, string $query, $conversations = [], int $topK = 5): array
    {
        try {
            $expandedQueries = $this->expandQueryWithAI($query, $conversations);
            ...
            foreach ($expandedQueries as $expandedQuery) {
                ...
                foreach ($chunks as $chunk) {
                    ...
                    if (!isset($allResults[$chunkId]) || $allResults[$chunkId]['similarity'] < $similarity) {
                        $allResults[$chunkId] = [
                            'chunk' => $chunk,
                            'similarity' => $similarity,
                            'matched_query' => $expandedQuery
                        ];
                    }
                }
            }
    ...
    private function expandQueryWithAI(string $query, $conversations = []): array
    {
        try {
            ...
            $conversations[] = [
                'role'      => 'system',
                'content'   => question_optimize($query),
            ];
            ...
            if (!$response->successful()) {
                Log::warning('Query expansion failed, using original query');
                return [$query];
            }
            ...
            Log::error('Query expansion error: ' . $e->getMessage());
