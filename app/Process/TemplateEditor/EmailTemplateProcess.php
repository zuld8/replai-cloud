...
class EmailTemplateProcess extends Process
{

    public function showEditorTemplate(Request $request, MessageTemplate $model)
    {
        $editorConfig = app(Config::class)->initialize($model); 
        return view('master.email.update', compact('editorConfig', 'model'));
    }
    ...
    { 
        ...
        return collect([
            ...
            ->map(function($type) use ($model){
                ...
                $base_path_project_views = resource_path('views/master/email/');
                ...
                foreach (File::allFiles($path) as $fileInfo) {
                    ...
                    $content = view("master.email.{$view_base}{$file_name}")->render();
