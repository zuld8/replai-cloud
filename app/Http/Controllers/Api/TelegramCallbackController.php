...
class TelegramCallbackController extends Controller
{
    ...
    */
    ...
    private function createUserMessage($histories, TelegramProcessorService $processor): object
    {
        ...
        // Extract quoted reply from Telegram (reply_to_message)
        $tgReplyTo   = null;
        $tgReplyText = null;
        ...
        if ($tgReplyMsgId) {
            ...
            if ($originalMsg) {
                ...
                // Fallback: use raw text from Telegram payload
                $tgReplyText = request()->input('message.reply_to_message.text')
                            ?? request()->input('message.reply_to_message.caption')
                            ?? null;
            }
        }
