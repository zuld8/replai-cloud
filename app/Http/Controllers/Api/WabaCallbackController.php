...
class WabaCallbackController extends Controller
{
    ...
    */
    ...
    /**
     * Generic webhook verify for Meta App Dashboard
     */
    public function genericWebhookVerify(Request $request)
    {
        $verifyToken = 'replai_verify_2026';
    ...
    /**
     * Generic webhook callback - routes to correct device
     */
    public function genericWebhookCallback(Request $request)
    {
        // Route to correct business based on WABA ID in payload
        try {
            $data = $request->all();
            ...
            // Find device by WABA ID
            $device = \App\Models\WhatsappKeyAccount::whereRaw(
                "JSON_UNQUOTE(JSON_EXTRACT(meta_data, '$.whatsapp.waba_id')) = ?", [$wabaId]
            )->where('status', 'active')->first();

            if (!$device) {
                return response()->json(['status' => 'ok', 'note' => 'device_not_found']);
            }

            $settings = \App\Models\Setting::withoutGlobalScopes()->find($device->business_id);
            if (!$settings) {
                return response()->json(['status' => 'ok']);
            }

            // Delegate to callbackMessage logic
            $fakeRequest = $request;
            return $this->callbackMessage($fakeRequest, $settings->id);

        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('Generic webhook error: ' . $e->getMessage(), [
                'file' => $e->getFile() . ':' . $e->getLine(),
            ]);
            // ── Handle phone_number_update (Coexistence mode change) ──
            try {
                $entry   = $data['entry'][0] ?? [];
                $changes = $entry['changes'][0] ?? [];
                if (($changes['field'] ?? '') === 'phone_number_update') {
                    $phoneNumberId = $changes['value']['phone_number_id'] ?? null;
                    $accountMode   = $changes['value']['account_mode'] ?? null;
                    if ($phoneNumberId && $accountMode) {
                        $dev = \App\Models\WhatsappKeyAccount::whereRaw(
                            "JSON_EXTRACT(meta_data, '$.whatsapp.phone_number_id') = ?",
                            [$phoneNumberId]
                        )->first();
                        if ($dev) {
                            $m = json_decode($dev->meta_data, true) ?? [];
                            $m['whatsapp']['account_mode'] = $accountMode;
                            $dev->meta_data = json_encode($m);
                            $dev->save();
                            \Log::info('WA Hybrid: account_mode updated', [
                ...
                \Log::warning('phone_number_update handler error: ' . $e->getMessage());
            }
    ...
    public function callbackMessage(Request $request, string $settingsId)
    {
        ...
        try {
            $settings = Setting::withoutGlobalScopes()->findOrFail($settingsId);
        } catch (\Throwable $e) {
            Log::warning('WABA Callback: settings not found', ['id' => $settingsId, 'err' => $e->getMessage()]);
            return response()->json(['status' => 'ok', 'note' => 'settings_not_found'], 200);
        }
        ...
        try {
            ...
            // ── Handle reaction messages (emoji reactions) ──
            if ($messageData['messageType'] === 'reaction') {
                $reactionData = $messageData['rawMessage']['reaction'] ?? [];
                $reactionData['from'] = $messageData['from'] ?? $messageData['bsuid'];
                $phoneNumberId = $data['entry'][0]['changes'][0]['value']['metadata']['phone_number_id'] ?? '';
                $this->handleReaction($reactionData, $device->id, $phoneNumberId);
                return response()->json(['status' => 'ok']);
            }
            ...
            // ✅ Return 200 to stop Meta from retrying (retry storm = DDoS ourselves)
            // ✅ Log minimal info only (avoid memory error when $request->all() is large)
            Log::error('WABA Callback Error: ' . $e->getMessage() . ' [' . get_class($e) . ']', [
                'file'  => $e->getFile() . ':' . $e->getLine(),
                'url'   => $request->url(),
            ]);
    ...
    {
        $displayPhone = $data['entry'][0]['changes'][0]['value']['metadata']['display_phone_number'] ?? '';

        // Normalize: strip +, spaces, dashes -> "6285111317766"
    ...
    private function parseMessageData(array $data): ?array
    {
        ...
        return [
            ...
            'fromName'    => $detailContact['profile']['name'] ?? $waUsername ?? 'Unknown',
    ...
    {
        // Retry up to 3 times on deadlock (SQLSTATE 40001 / error 1213)
        $maxRetries = 3;
        $attempt    = 0;
        ...
        while ($attempt < $maxRetries) {
            ...
            try {
                ...
                // Deadlock: retry after short delay
                $isDeadlock = $e->getCode() == 40001 || str_contains($e->getMessage(), '1213');
                if ($isDeadlock && $attempt < $maxRetries) {
                    Log::warning('WABA processMessage deadlock, retrying', [
                        'attempt'    => $attempt,
                        'message_id' => $messageData['messageId'] ?? 'unknown',
                    ]);
                    usleep(100000 * $attempt); // 100ms, 200ms, 300ms
                    continue;
                }
                // Non-deadlock DB error or max retries reached — log & absorb
    ...
    private function getOrCreateHistory(array $messageData, WhatsappKeyAccount $device): mixed
    {
        ...
        $waUsername  = $messageData['wa_username'] ?? null;

        try {
            // BSUID Support (Meta March–June 2026):
            // If phone is hidden, use bsuid as unique identifier
            if ($fromPhone) {
                $uniqueAttributes = [
                    'device_id'        => null,
                ...
                // Phone hidden — identify by BSUID
                $uniqueAttributes = [
                    'device_id'        => null,
                    'bsuid'            => $bsuid,
                    'type'             => 'personal',
            ...
            $creationValues = [
                'name'        => $messageData['fromName'] ?? $waUsername ?? 'WhatsApp User',
                'from_number' => $fromPhone,
                'bsuid'       => $bsuid,
                'wa_username' => $waUsername,
            ...
            $history = HistoryChat::firstOrCreate($uniqueAttributes, $creationValues);
            ...
            if ($e->getCode() == 23000 || strpos($e->getMessage(), 'Duplicate entry') !== false) {
                Log::warning('Duplicate WABA history caught, fetching existing', [
                    'waba_id'     => $device->id,
                    'from_number' => $fromPhone,
                    'bsuid'       => $bsuid,
                    'error'       => $e->getMessage()
                ...
                $query = HistoryChat::where('whatsapp_waba_id', $device->id)
                    ->where('type', 'personal')
                    ->where('from', 'waba');

                if ($fromPhone) {
                    $query->where('from_number', $fromPhone);
                } else {
                    $query->where('bsuid', $bsuid);
                }

                return $query->firstOrFail();
            }

            Log::error('Database error in WABA getOrCreateHistory', [
                'error' => $e->getMessage(),
    ...
    private function parseMessageContent(array $rawMessage): array
    {
        ...
        switch ($messageType) {
            ...
            case 'sticker':
                $mediaId = $rawMessage['sticker']['id'] ?? null;
    ...
    private function handleMediaDownload(WhatsappKeyAccount $device, array $messageContent, string $messageType): array
    {
        ...
        try {
            $mediaMetadata = $this->getMediaMetadata($device, $messageContent['mediaId']);

            if (!$mediaMetadata) {
                throw new \Exception('Failed to get media metadata');
            }

            $mimeType = $messageContent['mimeType'] ?? 'application/octet-stream';
            $estimatedSize = $mediaMetadata['size'] ?? 0;

            $setting = $device->business;
            $storageCheck = $this->checkStorage($setting, $estimatedSize);
            ...
            $fileContent = $this->downloadMediaContent($device, $mediaMetadata['url']);

            if (!$fileContent) {
                throw new \Exception('Failed to download media content');
            }

            $actualSize = strlen($fileContent);

            $finalCheck = $this->checkStorage($setting, $actualSize);
            ...
            $fileName = uniqid('waba_', true) . '.' . $extension;
            ...
            $messageType = $this->getMessageTypeFromMime($actualMimeType);

            // Preserve 'sticker' type — webp mime returns 'image' but must stay 'sticker'
            if ($messageContent['messageType'] === 'sticker') {
                $messageType = 'sticker';
            }
            ...
            Log::error('WhatsApp media download error: ' . $e->getMessage(), [
                'media_id' => $messageContent['mediaId'] ?? null,
                'trace' => $e->getTraceAsString()
            ]);
        }
    ...
    private function downloadMediaContent(WhatsappKeyAccount $device, string $mediaUrl): ?string
    {
        try {
            if (!$mediaUrl) {
                return null;
            ...
            $response = Http::retry(2, 3000)->timeout(45)
            ...
            Log::error('Error downloading media content: ' . $e->getMessage());
            return null;
        }
    }


    private function getMediaMetadata(WhatsappKeyAccount $device, string $mediaId): ?array
    {
        try {
            $config = $device->meta_data ? json_decode($device->meta_data, true) : [];
            $accessToken = $config['whatsapp']['access_token'] ?? null;
            ...
            if (!$accessToken || !$mediaId) {
                Log::warning('Missing access token or media ID');
                return null;
            }

            $response = Http::retry(3, 2000)->timeout(15)
                ->withToken($accessToken)
                ->get("https://graph.facebook.com/v22.0/{$mediaId}");

            if (!$response->successful()) {
                Log::error('Failed to get media metadata', [
            ...
            return [
                ...
                'size' => $data['file_size'] ?? 0, // Size in bytes
                'sha256' => $data['sha256'] ?? null,
            ];
        } catch (\Throwable $e) {
            Log::error('Error getting media metadata: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectIntentWithGemini(
        ...
        try {
            ...
            Log::error('Gemini intent detection error: ' . $e->getMessage());
            return ['success' => false, 'intent' => null, 'credit_used' => 0];
        }
    }
    ...
    private function processAIReply(WhatsappKeyAccount $device, Setting $settings, $histories, array $messageContent): mixed
    {
        ...
        if ($creditUsed > 0) {
            $this->updateCredits($creditUsed, $device, $fineTunnel, null);
        }
    ...
    private function calculateCreditUsage(int $creditUsed, string $modelAi, $setting): int
    {
        ...
        if ($ai_option === 'gemini') {
            return $this->geminiAiServiceObserver->calculateCompletions(
                $modelAi,
                $creditUsed,
                0 // output tokens, bisa disesuaikan kalau ada
            );
        }
    ...
    private function calculateActualCredit(int $creditUsed, string $modelAi): int
    {
        $setting = InternalSetting::first(['credit_token_basic', 'credit_token_advance']);
        return $this->calculateCreditUsage($creditUsed, $modelAi, $setting);
    }
    ...
    private function notifyAgents(WhatsappKeyAccount $device, $histories, string $message): void
    {
        ...
        $agentIds = array_filter(array_map('trim', explode(',', $agentString)));
        // N+1 FIX: bulk load semua agents sekaligus (1 query vs N queries)
        $agents = User::whereIn('id', $agentIds)->get()->keyBy('id');
        foreach ($agentIds as $agentId) {
            $user = $agents->get($agentId);
            if ($user) {
    ...
    */
    ...
    {
        $metadata = $this->getMediaMetadata($device, $mediaId);
        if (!$metadata) {
            return null;
        }

        $content = $this->downloadMediaContent($device, $metadata['url']);
        if (!$content) {
            return null;
        }
    ...
    */
    ...
    /**
     * Check if webhook payload contains delivery status updates
     */
    private function hasStatusUpdates(array $data): bool
    {
        return isset($data['entry'][0]['changes'][0]['value']['statuses']);
    }

    /**
     * Process delivery status updates from Meta webhook
     * Meta sends: sent, delivered, read, failed statuses
     */
    private function processStatusUpdate(array $data, Setting $settings): void
    {
        try {
            ...
            foreach ($statuses as $statusUpdate) {
                ...
                // Map Meta status to our delivery_status enum
                ...
                // Update blash_details by wamid - only allow forward transitions
                $updated = DB::table('blash_details')
                    ->where('wamid', $wamid)
                    ->whereIn('delivery_status', $this->getAllowedTransitions($deliveryStatus))
                    ->update([
                ...
                // If failed, also update sending_status to 'no' for backward compatibility
                if ($deliveryStatus === 'failed' && $updated > 0) {
                    DB::table('blash_details')
                        ->where('wamid', $wamid)
                        ->update([
                            'sending_status' => 'no',
                            'reports' => $errorInfo ?? 'Message delivery failed',
                        ]);
    ...
    /**
     * Status transitions: only allow forward transitions
     * queued -> sent -> delivered -> read
     * any -> failed
     */
    private function getAllowedTransitions(string $newStatus): array
    {
    ...
    /**
     * Handle incoming reaction webhook from WhatsApp
     */
    protected function handleReaction(array $reaction, string $wabaId, string $phoneNumberId): void
    {
        try {
            $messageId = $reaction['message_id'] ?? null;  // The message that was reacted to
            $emoji = $reaction['emoji'] ?? null;
            $fromNumber = $reaction['from'] ?? null;

            if (!$messageId) {
                \Log::info('Reaction: No message_id', $reaction);
                return;
            }

            // Find the message detail by messageid (WhatsApp message ID)
            $detail = \App\Models\ChatBot\HistoryChatDetail::where('messageid', $messageId)->first();
            
            if (!$detail) {
                \Log::info('Reaction: Message not found for wamid: ' . $messageId);
                return;
            }

            if (empty($emoji)) {
                // Empty emoji = reaction removed
                \App\Models\ChatBot\MessageReaction::where('message_wamid', $messageId)
                    ->where('reactor_phone', $fromNumber)
                    ->delete();
                \Log::info('Reaction removed', ['wamid' => $messageId, 'from' => $fromNumber]);
            } else {
                // Upsert reaction
                \App\Models\ChatBot\MessageReaction::updateOrCreate(
                    [
                        'message_wamid' => $messageId,
                        'reactor_phone' => $fromNumber,
                    ],
                    [
                        'history_chat_detail_id' => $detail->id,
                        'emoji' => $emoji,
                        'reactor_type' => 'customer',
                    ]
                );
                \Log::info('Reaction saved', ['wamid' => $messageId, 'emoji' => $emoji, 'from' => $fromNumber]);
            }
        } catch (\Throwable $e) {
