...
class MessengerController extends Controller
{
    ...
    private function processWebhook(Request $request)
    {
        try {
            ...
            // Instagram DMs are sometimes routed through the Page webhook
            // Detect and forward to Instagram controller
            if (isset($data['object']) && $data['object'] === 'instagram') {
                return app(\App\Http\Controllers\Api\InstagramController::class)->handle($request);
            }
    ...
    {
        $message = $messageContent['message'];
        $method = $messengerAccount->auto_reply_method;
        ...
        if ($method === 'all') { 
            if ($this->shouldUseChatbot($messengerAccount, $histories)) {
                $chatbotReply = $this->processChatbotReply($messengerAccount, $histories, $message);
    ...
    private function detectIntentWithGemini(
        ...
        try {
            ...
            Log::error('Messenger Gemini intent detection error: ' . $e->getMessage());
            return ['success' => false, 'intent' => null, 'credit_used' => 0];
        }
    }
