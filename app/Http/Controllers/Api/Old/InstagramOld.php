...
class InstagramController extends Controller
{
            ...
            foreach ($data['entry'] as $entry) {
                ...
                foreach ($entry['messaging'] as $messaging) {
                    if (isset($messaging['delivery'])) {
                        Log::info('Instagram delivery report received', [
                    ...
                    if (isset($messaging['message']['is_echo']) && $messaging['message']['is_echo'] === true) {
                        Log::info('Instagram echo message skipped', [
    ...
    private function getSenderInfo(InstagramAccount $instagramAccount, string $senderId): array
    {
        try {
            ...
            Log::warning('Instagram sender info API failed', [
                'sender_id' => $senderId,
    ...
    private function handleMediaDownload(InstagramAccount $instagramAccount, array $messageContent, string $messageType): array
    {
        ...
        try {
            ...
            Log::info("Instagram media stored: {$publicPath}");
        } catch (\Exception $e) {
    ...
    private function processAIReply(InstagramAccount $instagramAccount, $histories, array $messageContent): mixed
    {
        ...
        return $this->handleAIResponse($responseBody, $instagramAccount, $histories, $fineTunnel);
    }

    /**
     * Handle AI response based on intent
     */
    private function handleAIResponse($responseBody, InstagramAccount $instagramAccount, $histories, $fineTunnel): mixed
    {
