...
class BillingController extends Controller
{
    ...
    public function __construct(TransactionObserver $transactionObserver, NotificationObserver $notificationObserver, WhatsappServiceObserver $whatsappServiceObserver, BankObserver $banksObserver, DuitkuObserver $duitkuObserver)
    {
    ...
    public function index(Request $request)
    {
        ...
        $internal       = InternalSetting::first(['token_per_price', 'price_token']);
        $transactions   = $this->transactionObserver->getData($request, 'topup')->where('business_id', $settings->id)->get();

        $usingTopUp     = $settings->package_active_topup ? (int)$settings->package_active_topup->using_credit_limit : 0;
        $usingPackage   = $settings->package_active ? (int)$settings->package_active->using_credit_limit : 0;
        $packageCredit  = $settings->package_active ? (int)$settings->package_active->ai_response : 0;

        $topUpCredit    = $settings->package_active_topup ? (int)$settings->package_active_topup->ai_response : 0;

        $totalCredit    = $packageCredit + $topUpCredit;
        $usingCredit    = $usingTopUp + $usingPackage;
        $percentage     = $totalCredit > 0 ? ($usingCredit / $totalCredit) * 100 : 0;

        $summaries  = [
            'package_credit' => ($packageCredit - $usingPackage) < 1 ? 0 : ($packageCredit - $usingPackage),
            'top_up_credit'  => ($topUpCredit - $usingTopUp) < 0 ? 0 : ($topUpCredit - $usingTopUp),
            'using_credit'   => $usingCredit,
            'percentase'     => round($percentage, 2)
        ];

        return view('billing.index', ['page' => 'Billing Information'], compact('summaries', 'transactions', 'internal'));
    }
    ...
    public function detail(Request $request, PackageTransaction $transaction)
    {
        ...
        return view('billing.detail', ['page'    => 'TopUp Ai Response'], compact('transaction', 'banks', 'payments', 'setting'));
    }
    ...
    public function createTopUpTransaction(Request $request)
    {
        ...
        if ($business->package_transaction_topup_pending == false) {
        ...
        try {
            ...
            $transaction        = $this->transactionObserver->createTopUp($business, $request);
    ...
    public function payTransaction(Request $request, PackageTransaction $transaction)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('billing.index')->with(['flash'    => __('general.success_add_data')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with(['gagal' => $e->getMessage()]);
        }
