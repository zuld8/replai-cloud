...
class MessageBillingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $internal       = InternalSetting::first(['message_per_price', 'price_message']);
        ...
        $activeTopup    = $settings->package_active_message; // Ini khusus untuk type='mua'
        ...
        $packageMuaLimit = $activePackage ? (int)$activePackage->message_limit : 0;
        $topupMuaLimit   = $activeTopup ? (int)$activeTopup->message_limit : 0;
        ...
        $summaries = [
            'package_message_limit' => $packageMuaLimit,
            'topup_message_limit'   => $topupMuaLimit,
            'total_message_limit'   => $totalMuaLimit,
            'message_used' => $currentMuaUsage,
            'sisa_message_limit'    => $sisaMuaLimit,
            'percentage'        => round($percentage, 2)
        ];

        return view('billing.message-index', ['page' => 'Kredit Pesan Billing'], compact('summaries', 'transactions', 'internal'));
    }
    ...
    public function detail(Request $request, PackageTransaction $transaction)
    {
        ...
        return view('billing.message-detail', ['page' => 'TopUp Kredit Pesan'], compact('transaction', 'banks', 'payments', 'setting'));
    }
    ...
    public function createTokenDuitku(Request $request, PackageTransaction $transaction)
    {
        ...
        $data = [
            ...
            'productDetails'    => 'TopUp Kredit Pesan ' . (int)$transaction->new_order_message_limit,
            ...
            'itemDetails'       => [
                [
                    'name'          => 'TopUp Kredit Pesan ' . (int)$transaction->new_order_message_limit,
                    'price'         => $amount,
                    'quantity'      => 1
                ]
            ],
            ...
            'returnUrl'         => route('message.billing.detail', $transaction->id),
    ...
    public function createTransaction(Request $request)
    {
        ...
        if ($business->package_transaction_message_pending == false) {
        ...
        try {
            ...
            $transaction = $this->transactionObserver->createMessage($business, $request);
    ...
    public function payTransaction(Request $request, PackageTransaction $transaction)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('message.billing.index')->with(['flash' => __('general.success_add_data')]);
