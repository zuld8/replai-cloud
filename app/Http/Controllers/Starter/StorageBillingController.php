...
class StorageBillingController extends Controller
{
    ...
    public function __construct(TransactionObserver $transactionObserver, NotificationObserver $notificationObserver, WhatsappServiceObserver $whatsappServiceObserver, BankObserver $banksObserver, DuitkuObserver $duitkuObserver, PricingObserver $pricingObserver)
    {
    ...
    public function index(Request $request)
    {
        ...
        $transactions   = $this->transactionObserver->getData($request, 'storage')->where('business_id', $settings->id)->get();
        $addons         = $this->pricingObserver->getData($request, 'storage')->get();

        $storageFromSubscribe   = $settings->package_active ? (int)$settings->package_active->storage : 0;
        $storageFromAddons      = $settings->package_active_storage ? (int)$settings->package_active_storage->storage : 0;
        $storageAddonsId        = $settings->package_active_storage ? $settings->package_active_storage->id : null;

        $totalStorage           = $storageFromSubscribe + $storageFromAddons;
        $usingStorage           = $this->cacheStorageBusiness(my_business());

        // Perbaikan perhitungan persentase
        $percentase = $totalStorage > 0 ? round(($usingStorage / $totalStorage) * 100, 2) : 0;
        $remainingStorage = max(0, $totalStorage - $usingStorage);

        // Menentukan status storage
        $storageStatus = 'safe';
        if ($percentase >= 90) {
            $storageStatus = 'critical';
        } elseif ($percentase >= 70) {
            $storageStatus = 'warning';
        }

        $summaries  = [
            'storage_from_subscribe'    => $storageFromSubscribe,
            'storage_from_addons'       => $storageFromAddons,
            'total_storage'             => $totalStorage,
            'using_storage'             => $usingStorage,
            'remaining_storage'         => $remainingStorage,
            'percentase'                => $percentase,
            'storage_status'            => $storageStatus
        ];

        return view('storage.index', ['page' => 'Billing Information'], compact('summaries', 'transactions', 'storageAddonsId', 'addons'));
    }

    public function cacheStorageBusiness($businessId)
    {
        return Cache::remember("storage_usage_business_{$businessId}", 60, function () use ($businessId) {
            $totalSize = 0;
    ...
    public function createTransaction(Request $request, Package $package)
    {

        $business           = Setting::where('id', my_business())->first(['id', 'merchant_id']);

        if ($business->package_transaction_storage_pending->count() > 0) {
        ...
        try {
            ...
            $transaction        = $this->transactionObserver->createStorage($business, $package);
            ...
            if ($this->notificationObserver->whatsapp_buy_package == 'yes' && $this->notificationObserver->device) {
                if ($this->notificationObserver->buy_package_template_whatsapp) {
                    ...
                    $message    = str_replace(
                        ...
                        [($transaction->business->name ?? ''), my_user()->name, 'TopUp Ai Response', number_format($transaction->final_total), substr($transaction->created_at, 0, 10), $internalSetting->app_name],
                        $message
                    );
            ...
            return redirect()->back()->with(['flash' => 'Transaksi berhasil di buat']);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function detail(Request $request, PackageTransaction $transaction)
    {
        ...
        return view('storage.detail', ['page'    => 'TopUp Ai Response'], compact('transaction', 'banks', 'payments', 'setting'));
    }
    ...
    public function createTokenDuitku(Request $request, PackageTransaction $transaction)
    {
        ...
        $data = [
            ...
            'productDetails'    => 'TopUp Ai Credit ' . (float)$transaction->new_order_ai_response,
            ...
            'itemDetails'       => [
                [
                    'name'          => 'TopUp Ai Credit ' . (float)$transaction->new_order_ai_response,
                    'price'         => $amount,
                    'quantity'      => 1
                ]
            ],
            ...
            'returnUrl'         => route('billing.detail', $transaction->id),
    ...
    public function payTransaction(Request $request, PackageTransaction $transaction)
    {
        ...
        try {
            ...
            if ($this->notificationObserver->whatsapp_package_payment == 'yes' && $this->notificationObserver->device) {
                if ($this->notificationObserver->package_payment_template_whatsapp) {
                    ...
                    $message    = str_replace(
                        ...
                        [($transaction->merchant->name ?? ''), my_user()->name, 'TopUp Ai Response', number_format($payment->amount), substr($transaction->created_at, 0, 10), $internalSetting->app_name, $payment->bank_name, ($payment->bank->name ?? '')],
                        $message
                    );
            ...
            DB::commit();
            return redirect()->route('storage.index')->with(['flash'    => __('general.success_add_data')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with(['gagal' => $e->getMessage()]);
        }
