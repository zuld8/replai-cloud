...
use Exception;
use App\Events\EnvirontmentSaved;
...
class EnvironmentController extends Controller
{
    ...
    public function saveClassic(Request $input, Redirector $redirect)
    {
        ...
        event(new EnvirontmentSaved($input));

        return $redirect->route('environmentClassic')
    ...
    public function saveWizard(Request $request, Redirector $redirect)
    {
        ...
        if ($validator->fails()) {
            return $redirect->route('environmentWizard')->withInput()->withErrors($validator->errors());
        }

        if (! $this->checkDatabaseConnection($request)) {
            return $redirect->route('environmentWizard')->withInput()->withErrors([
        ...
        event(new EnvirontmentSaved($request));
