...
trait EditorWebTrait
{
    ...
    protected function findAndSetPlaceholders($html)
    {
        ...
        foreach ($placeholders as $_placeholder) {
            if (empty($this->placeholders[$_placeholder])) {
                ...
                if (view()->exists("mail.placeholders.{$view}")) {
                    $this->setPlaceholder($_placeholder, view("mail.placeholders.{$view}", $attributes)->render());
                }
            }
        }
    ...
    {
        $extrackBody    = json_decode($this->content);
        $html           = $extrackBody->html ?? '';
    ...
    {
        $extrackBody    = json_decode($this->content);
        $components     = json_decode($extrackBody->components ?? '[]');
        return $components;
    }
    ...
    {
        $extrackBody    = json_decode($this->content);
        $styles         = json_decode($extrackBody->styles ?? '[]');
        return $styles;
    }
    ...
    {
        return route('pages.edit', $this->getKeyValue());
    }

    public function getTemplatesUrlAttribute(): ?string
    {
        return route('pages.components', $this->getKeyValue());
    }
