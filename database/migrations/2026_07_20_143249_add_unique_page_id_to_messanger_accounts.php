...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $dupes = \DB::table('messanger_accounts')
            ->select('page_id')
            ->groupBy('page_id')
            ->havingRaw('COUNT(*) > 1')
            ->pluck('page_id');
        ...
        $exists = collect(\DB::select(
            "SHOW INDEX FROM messanger_accounts WHERE Key_name = 'messanger_accounts_page_id_unique'"
        ))->isNotEmpty();

        if (!$exists) {
            Schema::table('messanger_accounts', function (Blueprint $table) {
                ...
                $table->unique('page_id', 'messanger_accounts_page_id_unique');
            });
        }
    }
    ...
    {
        Schema::table('messanger_accounts', function (Blueprint $table) {
            $table->dropUnique('messanger_accounts_page_id_unique');
        });
    }
};
