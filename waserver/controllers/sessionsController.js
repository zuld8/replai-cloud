...
const status = (req, res) => {
    fs.readFile(
        `waserver/sessions/md_${res.locals.sessionId}/creds.json`,
        function (err, data) {
            if (err) {
                const states = [
                    "connecting",
                ...
                state =
                    state === "connected" &&
                    typeof (session.isLegacy
                        ? session.state.legacy.user
...
const getContacts = async (req, res) => {
    ...
    getContactByDevice(session, id);
};

const add = (req, res) => {
    const { id, isLegacy, notification, history } = req.body;
    ...
    createSession(id, isLegacy === "true", res, notification, history);
};
...
export { find, status, add, del, getContacts };
